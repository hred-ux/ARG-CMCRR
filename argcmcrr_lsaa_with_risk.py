...
model_class_path = os.path.join(current_dir, "ptFile", "aa_long_class.pt")
model_class.load_state_dict(
    torch.load(model_class_path, map_location=DEVICE)
)
model_mech_path = os.path.join(current_dir, "ptFile", "aa_long_mechanism.pt")
...
tokenizer = AutoTokenizer.from_pretrained("/hy-tmp/github_script/esm2/esm2-650m", do_lower_case=False)
model_esm = AutoModel.from_pretrained("/hy-tmp/github_script/esm2/esm2-650m").to(DEVICE).eval()
model_esm = model_esm.half()

print("Loading XGBoost model...")
clf = XGBClassifier()
clf.load_model("/hy-tmp/github_script/risk_modelfile/xgb_esm2_650m.json")
le = joblib.load("/hy-tmp/github_script/risk_modelfile/label_encoder.pkl")
...
def run_blast_multi(
    ...
    cmd = [
        "/hy-tmp/github_script/blast/bin/blastp",
        "-query", query_fasta,
        "-db", db_fasta,
...
def lsaa_predict_with_risk(input_file, outfile):
    ...
    if len(arg_seqs) > 0:
        ...
        print(f"Start blast")
        blast21 = blast_all_at_once(arg_seqs, db_fasta="/hy-tmp/github_script/blastfile/ARG-Risk-21.fasta",
                                prefix="ARG-Risk-21")
        blast22 = blast_all_at_once(arg_seqs, db_fasta="/hy-tmp/github_script/blastfile/ARG-Risk-22.fasta",
                                prefix="ARG-Risk-22")
