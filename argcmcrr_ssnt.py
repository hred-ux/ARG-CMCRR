import torch
from utils import process_ssnt
...
model_class.load_state_dict(torch.load("/hy-tmp/github_script/ptFile/nt_short_class.pt",map_location=DEVICE))
model_mech.load_state_dict(torch.load("/hy-tmp/github_script/ptFile/mechanism_not_nonarg/nt_short_mechanism.pt",map_location=DEVICE))
...
def ssaa_predict(input_file, outfile):
    ...
    features, lengths, headers, seqs = process_ssnt(input_file)
